    ...
    unsigned int *hash_size)
{
    ...
    if (!ctx) {
        logger(LOG_ERR, "hash_sha3_512: EVP_MD_CTX_new: %s",
            osh_openssl_strerror);
        goto error;
    }
    if (!EVP_DigestInit_ex(ctx, EVP_sha3_512(), NULL)) {
        logger(LOG_ERR, "hash_sha3_512: EVP_DigestInit_ex: %s",
            osh_openssl_strerror);
        goto error;
    }
    if (!EVP_DigestUpdate(ctx, in, in_size)) {
        logger(LOG_ERR, "hash_sha3_512: EVP_DigestUpdate: %s",
            osh_openssl_strerror);
        goto error;
    }
    if (!EVP_DigestFinal_ex(ctx, hash, hash_size)) {
        logger(LOG_ERR, "hash_sha3_512: EVP_DigestFinal_ex: %s",
            osh_openssl_strerror);
        goto error;
    }
    EVP_MD_CTX_free(ctx);
